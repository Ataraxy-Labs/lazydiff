...
This is referenced from the active feature's `RULES.md` "Operating rule — finishing the work" (currently `docs/features/001-diff-workspace/RULES.md`) and from each issue's `verification` field in the active feature's `issues.json`. Do not skip it.
...
8. Commit per the active feature's `RULES.md` "Detailed commits per task."
```
